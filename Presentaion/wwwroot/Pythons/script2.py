...
filename = 'original_lane_detection_6.jpg'
edge_value = 80
edge2_value = 120
edge3_value = 120
corners = [0, 0.04, 0.35, 0.35, 0, 0, 0.55, 0.55]
...
class Lane:
  ...
  def __init__(self, orig_frame):
    ...
    self.roi_points = np.float32([
      (int(corners[2]*width),int(corners[6]*height)), # Top-left corner
      (corners[0]*width, (1 - corners[4])*height-1), # Bottom-left corner			
      (int((1 - corners[1])*width),(1 - corners[5])*height-1), # Bottom-right corner
      (int((1 - corners[3])*width),int(corners[7]*height)) # Top-right corner
    ])
  ...
  def get_lane_line_indices_sliding_windows(self, plot=False):
    ...
    #print('self.left_fit:{}, self.right_fit:{}, self.margin:{}'.format(self.left_fit, self.right_fit, self.margin))
    margin = self.margin
 
    frame_sliding_window = self.warped_frame.copy()
 
    # Set the height of the sliding windows
    window_height = np.int_(self.warped_frame.shape[0]/self.no_of_windows)       
    ...
    for window in range(no_of_windows):
      ...
      #print('win_y_low:{}, win_y_high:{}, win_xleft_low:{}, win_xleft_high:{}'.format(win_y_low, win_y_high, win_xleft_low, win_xleft_high))
      ...
      #print('good_left_inds:{}'.format(good_left_inds))
    ...
    #print('left_lane_inds:{}'.format(left_lane_inds))
    leftx = nonzerox[left_lane_inds]
    lefty = nonzeroy[left_lane_inds] 
    rightx = nonzerox[right_lane_inds] 
    righty = nonzeroy[right_lane_inds]
  ...
  def get_line_markings(self, frame=None):
    ...
    _, sxbinary = threshold(hls[:, :, 1], thresh=(edge2_value, 255))
    ...
    # White in the regions with the purest hue colors (e.g. > edge_value (default 80) ...play with
    # this value for best results).
    s_channel = hls[:, :, 2] # use only the saturation channel data
    _, s_binary = threshold(s_channel, (edge_value, 255))
    ...
    _, r_thresh = threshold(frame[:, :, 2], thresh=(edge3_value, 255))
  ...
  def histogram_peak(self):
    ...
    peak.
    """
    midpoint = np.int_(self.histogram.shape[0]/2)
...
def main():
  ...
  cv2.imwrite(new_filename + '_thresholded.jpg', lane_line_markings)
  cv2.imwrite(new_filename + '_output.jpg', frame_with_lane_lines2)
...
corners = [0.13, -0.42, 0.21, 0.51, 0, 0, 0.55, 0.55]
