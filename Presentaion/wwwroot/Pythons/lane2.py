...
filename = 'orig_lane_detection_3_1280_720.mp4'
file_size = (1280, 720)
...
class Lane:
  ...
  def __init__(self, orig_frame):
    ...
    self.roi_points = np.float32([
      (int(0.35*width),int(0.65*height)), # Top-left corner
      (int(0.25*width), height-1), # Bottom-left corner			
      (int(0.75*width),height-1), # Bottom-right corner
      (int(0.65*width),int(0.65*height)) # Top-right corner
    ])
  ...
  def get_lane_line_previous_window(self, left_fit, right_fit, plot=False):
    ...
    if len(prev_left_fit2) > frame_rate:
  ...
  def get_lane_line_indices_sliding_windows(self, plot=False):
   ...
   # print('left_lane_inds:{}'.format(left_lane_inds))
    ...
    if 1 == 1 or lefty != None and leftx != None:
        left_fit = np.polyfit(lefty, leftx, 2)
        right_fit = np.polyfit(righty, rightx, 2) 
        ...
        # Calculate the moving average	
        if len(prev_left_fit) > frame_rate:
          prev_left_fit.pop(0)
